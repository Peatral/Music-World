...
const TARGET_SPINNER_SCENE = preload("res://scenes/target_spinner.tscn")
const TUNEY_SCENE = preload("res://scenes/tuney.tscn")
...
func spawn_tuney(from: Vector2, to: Vector2, duration: float, show_marker: bool = true, bounce: bool = false):
	var marker
	if show_marker:
...
func _ready():
	...
	for key in keys:
		...
		var start_position = get_tuney_start_position(target_position, anim.value_track_interpolate(use_offset_track, key_time), anim.value_track_interpolate(offset_track, key_time))
